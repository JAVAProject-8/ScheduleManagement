...
public class MainFrame extends JFrame implements ActionListener {
	...
	//public MainFrame(User _u) {
		...
		tabbedPane.addTab("그룹 메인", new JPanel());
		tabbedPane.addTab("그룹 시간표", new JPanel());
		
		// DB로부터 개인 일정, 그룹 일정, 그룹원 정보를 받아올때 각각 객체 Vector or List or ArrayList로 받아와야 함
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("일정 추가")) {
			// JDialog
			// 일정 추가는 메뉴 외에 JTable 에서 MouseEvent로 호출될 수 있음
		}
			...
			String leaveGroupName = (String)JOptionPane.showInputDialog(null, "탈퇴할 그룹을 선택해주세요", "그룹 탈퇴", JOptionPane.QUESTION_MESSAGE, null, testArray, testArray[0]);
